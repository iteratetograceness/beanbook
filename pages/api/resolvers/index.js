  ...
  Query: {
    ...
    getEntry: async (root, args, context, info) => {
      try {
        ...
        const entry = await pool.query(query, [id]).then((res) => res.rows[0])
        return entry
      } catch (error) {
        return error
      }
    ...
    getSearch: async (root, args, context, info) => {
      ...
      if (!filters) {
        ...
        for (let filter of filters) {
          queryString += `UPPER(${filter}) LIKE UPPER($2) ${
            i === len ? '' : 'OR '
          }`
          i++
        }
    ...
    getRecentEntries: async (root, args, context, info) => {
      try {
        const userid = args.userid
        const query = `SELECT origin_name, favorited, rating, created_on, id FROM entries WHERE userid = $1 ORDER BY created_on DESC LIMIT 3`
        const entries = await pool.query(query, [userid])
